...
function szamol() {
    ...
    for(i in tomb) {
        tomb[i] = i > 0 ? tomb[i - 1] + Math.round(tomb[i - 1] * kamat / 100) : alap;
    }
...
function kiiras(arr) {
    fejlec = "<thead class='thead-dark'><tr><th scope='col'>Év</th><th scope='col'>Összeg</th></tr></thead><tbody>";
    const cucc = arr.reduce( (sorok, e, i) => sorok + '<tr><td>' + (i + 1) + '</td><td>' + e + '</td></tr>', fejlec);
	return cucc+"</tbody>";
}